...
import { BookMarked, ChevronDown, ChevronUp, Pencil, Trash2, X, Check, Plus, ArrowRightLeft } from 'lucide-react';
...
interface PrinciplesLogicsPanelProps {
  ...
  principlesMoveTargets: MoveTarget[];
  logicsMoveTargets: MoveTarget[];
  isOpen: boolean;
  onClose: () => void;
}
                        ...
                        {movingId === item.id && (
                          <div className="absolute right-0 top-5 bg-white border border-gray-100 rounded-xl shadow-lg z-50 py-1 min-w-[7.5rem]">
                            {moveTargets.map((target) => (
                              <button
                                key={target.label}
                                onClick={() => { target.onClick(item.id, item.content); setMovingId(null); }}
                                className={cn('flex items-center w-full px-3 py-2 text-xs text-gray-700 transition-colors', target.colorClass)}
                              >
                                → {target.label}
                              </button>
                            ))}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          <Section
            ...
            moveTargets={principlesMoveTargets}
          />
          <Section
            title="変換操作ロジック"
